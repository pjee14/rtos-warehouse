...
> 目前為**筆電模擬版**:七段顯示器 / LED / 蜂鳴器以 terminal 文字模擬。
...
5 條 task(pthread)以即時優先權並行運作,資料流如下:
    ...
    [Scanner]  掃描進出貨,從記憶體池取訊息,丟進優先序佇列
        |
...
| Task | 優先權 | 職責 |
|------|--------|------|
| Scanner | High (80) | 模擬掃描進出貨,封裝訊息丟進佇列 |
| Alert | High (80) | 收到庫存不足通知 → 控制 LED / 蜂鳴器 |
| Inventory | Medium (50) | 取訊息、更新庫存、判斷門檻 |
| Socket | Medium (50) | TCP server,接受遠端 client 查詢 |
| Display | Low (20) | 週期顯示庫存到七段顯示器 |
...
架構說明、簡報對應、Demo 腳本、預期問答,詳見 docs/ 資料夾。
